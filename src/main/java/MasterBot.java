 ...
 */
public class MasterBot extends SmartBot
{
    public MasterBot() {}

    public MasterBot(String name, int gold)
    {
        super(name, gold);
    }
    ...
    public void AI(List<Bot> bots, Round round)
    {
        ...
        if (round.getStage() == Stage.PREFLOP)
        {
            ...
            {
                if(StartingValue>22 && round.getInValue()<round.getBb()*8)
                {
            ...
            if(StartingValue>=35)
            {
                ...
                if (round.getInValue()>round.getBb()*34 || goldInGame > gold)
                {
            ...
            else if(isMediumPosition)
            {
                ...
                if (someoneRaised)
                {
                    ...
                    else if(StartingValue>=32) {why="Ktos podbil, ale flop moze duzo zmienic."; CheckIn(); return;}
                }
                else if(StartingValue>=24) {why="Nie ma podbicia, sprobuje zrobic steal"; Bet(round.getInValue() * 4);
                    return;}
            }
            else if(isEarlyPosition)
            {
                ...
                if (someoneRaised)
                {
                    ...
                    else if(StartingValue>=33) {why="Ktos podbil, ale flop moze duzo zmienic."; CheckIn(); return;}
                }
                else if(StartingValue>=30) {why="Nie ma podbicia, wiec wejdzmy z tym"; CheckIn(); return;}
            }
        }
        //======================================================================
        if(noRaiseCounter>2 && round.getStage() == Stage.RIVER && StartingValue >= 20)
        {
            why="Zablefuje! Arghh";
            Bet(round.getInValue()*3);
            return;
        ...
        if(round.getInValue()>goldInGame)
        {
            why="Dam Fold. Mam slabe przeczucie co do tej rundy";
            Fold();
            return;
        }
        else
        {
            why="Narazie poczekamy na nastepne karty"; CheckIn();
        }
